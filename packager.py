...
def usage():
  print("Usage: %s [-h|--help] --version version [--project-name name] [--build dirname] [--build-package] [--no-entry-changelog] [--distribution-name name] [--distribution-version version] [--distribution-id id] [--arch arch] [--package-distribution name] [--package-template dirname] [--force-target target] [--urgency urgency] [--authors authors] [--utc utc]" % sys.argv[0])
...
for o,a in options:
  if o in ("-h", "--help"):
    usage()
    sys.exit(1)
  elif o == "--build":                opts.dirname = a
  elif o == "--version":              opts.force_config["%VERSION%"] = a
  elif o == "--project-name":         opts.project_name = a
  elif o == "--build-package":        opts.build_package = True
  elif o == "--no-entry-changelog":   opts.entry_changelog = False
  ...
  elif o == "--urgency":              opts.urgency = a
  elif o == "--authors":              opts.authors = a
  elif o == "--utc":                  opts.utc = a

if not "%VERSION%" in opts.force_config:
...
def get_changelog_entry(project_name, version, authors, urgency, utc):
  ...
  if os.system("%s %s" % (os.environ['EDITOR'], tmp_changelog)):
    raise Exception("Error in `%s %s`" % (os.environ['EDITOR'], tmp_changelog))
  ...
  return "%s (%s%%TARGET_NAME%%) %%PKG_DISTRIBUTION%%; urgency=%s\n\n%s\n\n -- %s  %s\n\n" % (
    ...
    changelog,
    authors,
    datetime.datetime.today().strftime("%%a, %%d %%b %%Y %%H:%%M:%%S +%s" % utc)
  )
...
try:
  ...
  if not "%PKG_DISTRIBUTION%" in opts.config:
